...
include("NMFk-anasol1-20150721.jl")
conc = OrderedDict()
for key in sort(collect(keys(Points)))
	...
	md["Sources"][1]["box"]["x"]["init"] = welldata["x"] - 500
	md["Sources"][1]["box"]["t0"]["init"] = ( welldata["x"] - 500 ) / md["Parameters"]["vx"]["init"]
	md["Sources"][1]["box"]["t1"]["init"] = ( welldata["x"] - 500 ) / md["Parameters"]["vx"]["init"] + 1
	wells[key] = welldata
	md["Wells"] = wells
